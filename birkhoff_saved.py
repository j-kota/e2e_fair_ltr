...
from networkx.algorithms.bipartite.matching import maximum_matching, eppstein_matching   # JK
import numpy as np
import sys # JK
np.set_printoptions(threshold=sys.maxsize)  # JK
...
def birkhoff_von_neumann_decomposition(D):
    ...
    while not np.all(S == 0):
        ...
        left_nodes = range(n)
        #M = maximum_matching(G, left_nodes)   # JK
        M = eppstein_matching(G, left_nodes)
